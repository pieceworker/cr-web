...
async function getDirectors() {
    ...
    const db = env.DB;
    const directors = await db.prepare("SELECT * FROM users WHERE role = 'Chapter Director' ORDER BY name ASC").all();
    return (directors.results as unknown as User[] || []);
}

export default async function ChapterDirectorsPage() {
            ...
            <section className="space-y-12">
                <div className="space-y-4">
                    <h1 className="text-5xl md:text-7xl font-black uppercase italic text-zinc-900 dark:text-white font-heading leading-none tracking-tighter">
                        Chapter<br /><span className="text-red-600">Directors</span>
                    </h1>
                    <p className="text-zinc-500 font-medium italic max-w-2xl">The leadership driving the revolution in every city.</p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8">
                    {directors.length > 0 ? directors.map((m) => (
                        ...
                        <p className="col-span-full text-zinc-500 italic py-12 text-center uppercase tracking-widest border border-dashed border-zinc-200 dark:border-zinc-800">No directors listed.</p>
                    )}
                </div>
            </section>
        </div>
